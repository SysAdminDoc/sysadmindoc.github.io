...
Highest-priority workflow/research work after the T108 homepage SectionJumpNav pass:

1. `T109` -- iOS PWA install path and prefers-contrast follow-up.
2. `T110` -- Language-donut build/JS parity and skill-ring distribution alignment.
3. `T111` -- Root-cause Astro 6 HTML repair and convert the fixer to assert-or-noop when safe.

Next open checklist item in document order is `T109` iOS PWA install path and prefers-contrast follow-up.
