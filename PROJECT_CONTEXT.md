...
Highest-priority workflow/research work after the T112 terminal and Atom feed passes:

1. `T112` -- P3 cluster: no-JS catalog form, public JS minification, llms completeness, Beyond Code enrichments, style-src follow-up, and catalog DOM-size budget. Terminal commands and Atom feed are complete.

Next open checklist item in document order is the `T112` catalog no-JS `<form>` subitem.
