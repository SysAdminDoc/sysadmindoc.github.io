...
- `build:ci` minifies copied `dist/scripts/*.js` output with `scripts/minify-public-scripts.mjs` after Astro build. Source files under `public/scripts` stay readable; the minifier uses esbuild syntax/whitespace minification with identifier renaming disabled so classic-script globals shared across files remain stable.
...
- The homepage catalog search control is a real `GET` form with `action="/search/"` and input `name="q"`. JavaScript intercepts submit for in-page filtering and URL-backed `q=` state; no-JS users see the `<noscript>` submit button and land on `/search/?q=...`.
...
- Homepage Beyond Code uses a static `beyondCards` overview above the click-to-load drone videos. The music-generation context routes to `/projects/SlunderStudio/` instead of restoring the removed Spotify iframe/embed, preserving the site's lightweight/no-third-party-media posture. Homepage `main.js` restores initial and changed hashes after late layout work by revealing contained below-fold sections and repeating the fragment scroll, so `/#beyond` does not land in the catalog after profile-fed rendering.
...
- `/llms.txt` is an AI-readable site map generated from shared portfolio/page data. It includes featured projects, live apps, reviewed interior pages from `interiorOgPages`, the exact-count catalog anchor, language lanes from `LANGS`, feeds, JSON/script machine endpoints, sitemap, and a self-link. `npm run endpoints:audit` requires the expanded sections, required URLs, exact catalog count copy, and a minimum useful-link count.
...
- `/feed.json` is a JSON Feed 1.1 project feed with absolute `icon` and `favicon` URLs. `/atom.xml` mirrors the same project-feed population for Atom clients and is advertised through Base alternate links plus `llms.txt`. `npm run feed:audit` validates the built JSON and Atom feed metadata, icon assets, item IDs/URLs, Atom self-link and entry-count parity, content fields, uniqueness, and dates.
...
- `build:ci` runs Astro build, the HTML structure verifier, public script minification, public endpoint auditing, JSON/Atom Feed auditing, service-worker stamping, Pagefind indexing, the generated search audit, and the rendered JSON-LD audit. PR CI then runs an advisory Lighthouse CI budget after `build:ci`, publishes a compact warning table to the job summary, uploads filesystem reports plus `.tmp/lhci/summary.md`, and does not fail the job on budget warnings.
...
- Minify copied public scripts: `npm run scripts:minify` after `astro build`
...
- Audit built JSON/Atom Feed: `npm run feed:audit` after `npm run build`
...
- `npm run endpoints:audit` passed with 177 project-index rows, 60 release-index rows, 20 release repositories, 177 command-palette project rows, 9 quick links, 43 `llms.txt` links, 6 alternate discovery links, and 10 source header policies after the Atom feed addition.
- `npm run feed:audit` passed with 177 JSON Feed items, 177 `content_text` items, absolute `icon` `https://sysadmindoc.github.io/icon-512.png`, absolute `favicon` `https://sysadmindoc.github.io/favicon.svg`, and Atom entry-count parity.
- `npm run smoke:live -- --base-url https://sysadmindoc.github.io/ --expected-version 0.18.3 --expected-projects 177 --expected-releases 60 --expected-feed-items 177 --retries 2 --retry-ms 5000` passed against live Pages with the stamped `portfolio-v0.18.3` service worker, profile-feed-backed `/projects.json`, 60 releases, 177 JSON Feed items, release RSS, project RSS, `llms.txt`, `cmdk-data.js`, sitemap-index 200, and live `Cache-Control` `max-age=600`. The current script also checks `/atom.xml` entry-count parity after the T112 Atom pass.
...
- T112 Atom verification covered `node --check src/pages/atom.xml.ts`, `node --check scripts/audit-feed.mjs`, `node --check scripts/audit-public-endpoints.mjs`, `node --check scripts/smoke-live-site.mjs`, and `node --test test/atom-feed.test.mjs` before the full build/audit pass.
- T112 catalog no-JS form verification covered `node --check public/scripts/main.js`, `node --test test/catalog-noscript-form.test.mjs`, full unit/check/build/a11y gates, and local Playwright fallback probes after the in-app Browser attach timed out: JS submit stays on `/` with URL-backed `q=` state and no console warnings, while JavaScript-disabled submit navigates to `/search/?q=python` with no horizontal overflow.
- T112 public JS minification verification covered `node --check scripts/minify-public-scripts.mjs`, `node --test test/public-script-minify.test.mjs`, `npm run scripts:minify`, `npm test`, `$env:PROFILE_PROJECTS_OFFLINE='1'; npm run check`, `npm run build:ci`, minified `dist/scripts/*.js` parse checks, `npm run a11y:audit`, and local Playwright fallback smoke after the in-app Browser attach timed out. Current built `dist/scripts` output minifies 10 copied public scripts from 91.0 KB to 67.6 KB while preserving identifier names; minified home/search/project/resume/timeline routes loaded without console warnings, framework overlays, or horizontal overflow, and homepage catalog search/theme/command-palette plus timeline filter interactions worked.
- T112 `llms.txt` completeness verification covered `node --check scripts/audit-public-endpoints.mjs`, `node --test test/llms-completeness.test.mjs`, `node --test test/atom-feed.test.mjs`, and `npm run build:ci`. The built endpoint audit reported 59 `llms.txt` links and now enforces reviewed page, language lane, feed, machine endpoint, sitemap, and exact catalog-count coverage.
- T112 Beyond Code verification covered `node --test test/beyond-code.test.mjs`, `node --check public/scripts/main.js`, `npm test`, `$env:PROFILE_PROJECTS_OFFLINE='1'; npm run check`, `npm run build:ci`, `npm run a11y:audit`, and browser smoke of `/#beyond` on desktop/mobile. The Browser plugin path timed out attaching to the in-app webview, so standalone Playwright verified three desktop columns, one mobile column, the internal SlunderStudio card navigation, zero console warnings/errors, zero framework overlays, and zero horizontal overflow.
...
- 2026-06-04: Added the T112 Atom feed. `/atom.xml` now mirrors the project feed for Atom clients, is discoverable through Base alternate links and `llms.txt`, is precached beside `/rss.xml`, and is covered by endpoint, feed, smoke, and source-contract audits.
- 2026-06-04: Added the T112 catalog no-JS form. Homepage catalog search now submits as `GET /search/?q=...` without JavaScript while JS users stay on the homepage with immediate in-page filtering and URL-backed `q=` state.
