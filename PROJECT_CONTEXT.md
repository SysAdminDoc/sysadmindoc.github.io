...
- `build:ci` runs Astro build, the HTML structure verifier, public endpoint auditing, JSON/Atom Feed auditing, service-worker stamping, Pagefind indexing, the generated search audit, and the rendered JSON-LD audit. PR CI then runs an advisory Lighthouse CI budget after `build:ci`, publishes a compact warning table to the job summary, uploads filesystem reports plus `.tmp/lhci/summary.md`, and does not fail the job on budget warnings.
...
Highest-priority workflow/research work after the T112 terminal, Atom feed, and catalog no-JS form passes:

1. `T112` -- P3 cluster: public JS minification, llms completeness, Beyond Code enrichments, style-src follow-up, and catalog DOM-size budget. Terminal commands, Atom feed, and catalog no-JS form are complete.

Next open checklist item in document order is the `T112` public JS minification subitem.
