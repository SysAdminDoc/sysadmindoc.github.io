...
await fs.mkdir(astroThumbsDir, { recursive: true });
...
for (const slug of slugs) {
  ...
  const astroTarget = path.join(astroThumbsDir, `${slug}.jpg`);
  ...
  await Promise.all([
    fs.writeFile(target, thumbnail),
    fs.writeFile(astroTarget, thumbnail),
  ]);
...
console.log('  astro asset copies: src/assets/screenshots/thumbs/');
