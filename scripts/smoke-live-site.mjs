...
// The whole lead path, proven end to end; see scripts/lib/lead-delivery-check.mjs.
async function checkLeadDeliveryOnEdge(baseUrl, summary) {
  if (!isEdgeHost(baseUrl)) {
  ...
  await checkLeadDelivery({
    ...
    required: hasFlag('--require-lead-delivery'),
    runId,
    userAgent: `sysadmindoc-live-smoke/${runId}`,
    summary,
    fetch: timedFetch,
    sleep,
  });
