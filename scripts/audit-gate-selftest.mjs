...
const cases = [
  ...
  ...[
    ...
    // The fifteenth drain review's: the hand-written scanner missed these; parse5 doesn't.
    { kind: 'after an empty comment', form: 'a <style> after <!-->', file: 'privacy/index.html', plantIn: (/** @type {string} */ html) => html.replace(/(<\/head>)/i, '<!--><style>.planted{color:light-dark(#000,#fff)}</style>$1'), expect: /privacy\/index\.html <style> \d+: a light-dark/ },
    { kind: 'inline script', form: 'an inline script spelling it light\\x2ddark', file: 'privacy/index.html', plantIn: (/** @type {string} */ html) => html.replace(/(<\/body>)/i, '<script>document.head.insertAdjacentHTML("beforeend","<style>.p{color:light\\x2ddark(#000,#fff)}</style>")</script>$1'), expect: /privacy\/index\.html inline script \d+: a script carries a light-dark\(\)/ },
    { kind: 'data: stylesheet link', form: 'a stylesheet link to a data: URI', file: 'privacy/index.html', plantIn: (/** @type {string} */ html) => html.replace(/(<\/head>)/i, '<link rel="stylesheet" href="data:text/css,.p%7Bcolor:light-dark%28%23000,%23fff%29%7D">$1'), expect: /privacy\/index\.html data: stylesheet link \d+: a light-dark/ },
    { kind: 'SVG entity', form: 'an SVG that defines its own entities', file: 'favicon.svg', plantIn: (/** @type {string} */ svg) => `<!DOCTYPE svg [<!ENTITY ld "light-dark">]>\n${svg}`, expect: /favicon\.svg: it defines its own entities/ },
    { kind: 'unspaced @import', form: '@import"data:..." with no space', file: 'styles/offline.css', plantIn: (/** @type {string} */ css) => `@import"data:text/css,.p%7Bcolor:light-dark%28%23000,%23fff%29%7D";\n${css}`, expect: /styles\/offline\.css: in an @import of a data: URI, a light-dark/ },
