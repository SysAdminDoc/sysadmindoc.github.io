...
- [x] **Separate Playwright interaction and visual reports** -- Added a dedicated interaction-smoke Playwright config so `audit:interactions` writes `.tmp/playwright-interactions-*` while the visual/axe suite keeps `.tmp/playwright-*`, and CI uploads both report/result sets. _(Source: TODO.md T151, ROADMAP.md style-side CSP hardening)_
- [x] **Harden command-palette close behavior** -- Capture-phase Escape and Ctrl/Cmd+K handling now closes the native command-palette dialog from inside the search field, and the rendered interaction smoke verifies dialog state plus toggle/input `aria-expanded` updates. _(Source: TODO.md T152, ROADMAP.md style-side CSP hardening)_
- [x] **Guard command-palette keyboard activation** -- The rendered interaction smoke now verifies filtered result state, `aria-activedescendant`, Arrow selection, Enter activation of an internal route, and closed palette state after navigation. _(Source: TODO.md T153, ROADMAP.md style-side CSP hardening)_
- [x] **Guard command-palette pointer activation** -- The rendered interaction smoke now verifies native dialog backdrop dismiss, `aria-expanded` reset, pointer activation of an internal command result, navigation, and closed palette state after navigation. _(Source: TODO.md T154, ROADMAP.md style-side CSP hardening)_
- [x] **Guard command-palette no-results recovery** -- The rendered interaction smoke now verifies no-results status/copy, zero result rows, cleared active-descendant state, and recovery to valid command results. _(Source: TODO.md T155, ROADMAP.md style-side CSP hardening)_
- [x] **Split homepage interaction smoke flows** -- The rendered interaction suite now separates command-palette coverage from terminal/catalog/video homepage checks, keeping runtime-error and overflow guards on both focused flows. _(Source: TODO.md T156, ROADMAP.md style-side CSP hardening)_
