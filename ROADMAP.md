...
## Status: v0.18.3 implementation queue drained; Cycle 23 stabilized fixture-backed Playwright baselines
...
- Done in Cycle 23: `scripts/audit-public-endpoints.mjs` now derives the fixture `/llms.txt` useful-link floor from required URLs plus fixture project count while keeping the live-scale 50-link ceiling. `scripts/audit-dom-size.mjs` now uses a bounded `small-catalog` average-card budget for fixture output below 50 cards while preserving homepage, catalog, total-node, max-card, and live-scale ceilings. The eight Playwright screenshots were refreshed from the deterministic fixture `build:ci` output, and `PROFILE_PROJECTS_OFFLINE=1 npm run audit:playwright` passes all 21 CSP, axe, and visual checks.
- Confidence: High

### T146 - Promote style CSP hash drift checks into the build-output audit path

- Priority: P2
- Why: The active style policy depends on two SHA-256 hashes for intentional inline style blocks. Source tests guard those hashes, but the deploy build path should also fail if rendered `dist/` no longer matches the active `style-src-elem` contract.
- Evidence: `test/csp-audit.test.mjs` checks the `critical.css` hash and no-JS fallback hash in `Base.astro`; `node scripts/audit-csp.mjs --dist --candidate-style-src-elem "<active hashes>" --strict` passes after a build. `build:ci` does not yet run that strict rendered CSP check.
...
- Done in Cycle 29: Added `playwright.interactions.config.mjs` as a thin wrapper around the visual/axe audit config, keeping screenshot baseline paths inherited while writing interaction smoke reports to `.tmp/playwright-interactions-report` and results to `.tmp/playwright-interactions-results`. CI now uploads both interaction and visual report/result folders, and `test/a11y-gate.test.mjs` locks the separate paths.
- Confidence: High

### T152 - Harden command-palette close behavior

- Priority: P2
- Why: T149 exposed that the command-palette close contract is not currently guarded. Escape and Ctrl/Cmd+K are advertised in the UI, but the first interaction-smoke draft left the dialog open when those keys were pressed from inside the search field.
- Evidence: The failed rendered smoke attempts left `#cmdk` visible after `page.keyboard.press('Escape')` and after `page.keyboard.press('Control+K')` while the search input remained focused. Existing CSP/visual tests open and filter the palette but do not assert close behavior.
...
- Verify: `PROFILE_PROJECTS_OFFLINE=1 npm run build:ci`; `PROFILE_PROJECTS_OFFLINE=1 npm run audit:interactions`; `npm test`.
- Done in Cycle 32: Extended the rendered interaction smoke to click the native dialog backdrop and verify closed state plus `aria-expanded` reset, then reopen, filter a stable `search` result set, click the first internal command result, verify navigation, and confirm the palette is closed after navigation.
- Confidence: High

### T155 - Guard command-palette no-results recovery

- Priority: P2
- Why: Empty-result state is a user-facing command-palette path that can regress separately from successful filtering and activation. It should clear active option state and recover cleanly when the user edits back to a valid query.
- Evidence: `public/scripts/cmdk.js` renders `.cmdk-empty`, updates `#cmdkMeta`, and clears `aria-activedescendant` when no rows match. The rendered interaction smoke does not currently exercise the no-results branch.
- Touches: `tests/playwright/interaction-smoke.spec.mjs`, `public/scripts/cmdk.js` if reproduction exposes a behavior bug, `README.md`, and `PROJECT_CONTEXT.md`.
