...
- **Caddy image drift (Verified)**: `caddy:2.8-alpine` pinned in `deploy/vps/docker-compose.yml`; current stable is 2.11.2 (2026-03-06). 2026 CVEs fixed since 2.8: CVE-2026-27585 (path-matcher backslash bypass), CVE-2026-27586 (mTLS fail-open), CVE-2026-27589 (admin API), CVE-2026-27590 (FastCGI), header-injection privilege escalation (fixed 2.11.2). Frozen tag also means a stale Alpine base receiving no rebuilds. Edge Caddy (Contabo-VPS-Ops) has the same exposure.
...
- **sanitize-html upstream archived (Verified)**: apostrophecms/sanitize-html went read-only 2026-02-26. Installed 2.17.6 includes all 2026 fixes (CVE-2026-44990, -40186, -53606) but will never receive another patch. Usage here is build-time over reviewed/feed content (feeds, README excerpts), which bounds the risk; tighten `allowedSchemes`/URI attributes and document a DOMPurify+jsdom exit path.
