...
test('build:ci minifies copied public scripts', async () => {
  ...
  assert.match(pkg.scripts['build:ci'], /node scripts\/fix-html-structure\.mjs && npm run scripts:minify && npm run bundle:audit && npm run csp:audit:dist:style:elem && npm run endpoints:audit/);
