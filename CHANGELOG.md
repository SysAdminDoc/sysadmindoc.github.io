...
- Verified the data-health profile-feed pass with `npm run fetch-stars`, `npm run profile-feed:sync`, `npm run data:summary -- --out .tmp/data-refresh-t117 --max-age-hours 36 --fail-on-stale`, `npm test`, and `npm run check`; the summary reported profile feed status `active`, cache age 0h, 177 portfolio projects, and all profile-feed checks passing.
