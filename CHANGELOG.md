...
- Verified the performance/bfcache workflow pass locally with `node scripts/summarize-performance-audit.mjs .tmp/performance-audit-ci.json`, `npm test`, `npm run check`, `npm run build`, `npm run a11y:audit`, and `npm run audit:perf -- --base http://127.0.0.1:4321 --strict --lcp 60000 --event 500 --out .tmp/performance-audit-ci.json`; the strict audit passed all 5 route samples with bfcache restored and no overflow.
