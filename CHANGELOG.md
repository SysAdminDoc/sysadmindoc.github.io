...
- Every deploy now checks, on the running containers, that only the edge can choose the address the inner Caddy, ntfy and the contact handler see for a visitor. It reads the inner Caddy's config as Caddy adapted it, where a request header named through a placeholder counts too, and ntfy's command line and proxy environment, fails if ntfy has any config file at all, and checks that the report sink is still off their network. The check before shipping had its own reader for the Caddyfile and compose file, and five ways of writing a forged `X-Forwarded-For` got past it (an environment default, a snippet argument, `\\` before a quote, a continued line and a heredoc), as did ntfy's underscore flags, `NTFY_CONFIG_FILE` and a compose `extends`. That reader now follows Caddy's lexer on each of those, expands `{$...}` placeholders the way Caddy does whatever their name holds, and refuses what it can't follow, and it no longer fails a `header` block, which only changes responses, or a quoted flag value.
