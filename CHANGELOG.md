...
- The edge's own log no longer keeps a visitor's address or query string from a request to `/.well-known/acme-challenge/`. Caddy's warning for such a request carries the address and page at the top level, outside the `request` fields the filter cleaned, and the log held 13 of them on 2026-09-24. Both Caddy servers' filters now drop those top-level fields too, the deploy fails on a filter that doesn't, and a probe with a query on the live edge afterwards logged the path alone. The inner Caddy logs the same warning with the visitor's user agent, which its filter kept until the deploy started requiring both servers to drop the user agent and referrer.
