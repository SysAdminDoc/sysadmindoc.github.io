...
- Old links to the removed `/projects/<Repo>/` pages, the site's most common 404 (92 in the week to 2026-09-23), now go somewhere: a repo the catalog knows answers 301 to its GitHub page, and any other name 302s to a catalog search for it. The bare `/projects/` path, which was never a page, answers 301 to `/catalog/` instead of 404. `/favicon.ico` exists too, packed from the same 16, 32 and 48 pixel renders as the other icons. The live smoke checks each of these.
