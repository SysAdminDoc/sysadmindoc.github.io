...
- The contact form can't be flooded or scripted as easily. Its script now fetches a signed server timestamp once someone starts typing and sends it back with the message, and the handler refuses one that's under three seconds old, over four hours old, or already used, restarts included. That replaces the browser-clock check, which a missing field skipped entirely. Each visitor gets five attempts per ten minutes and ten a day, so one address can't fill the hourly cap by itself. A browser without JavaScript sends no token, so its post is taken only from a page on this site, with two attempts, and the site stores at most 30 messages an hour, even when they arrive at once. A form that fails a field check hears only "Please check the form and try again", and a filled honeypot is answered like a sent message, so a bot can't use the replies to find it.
