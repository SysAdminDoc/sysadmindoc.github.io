...
- The access log keeps only what the daily traffic report reads: the address, the page without its query string, the time, the method and protocol, the status, size and duration, the user agent and the referrer. Other request headers, the port, TLS details and response headers are no longer logged, and the log already on the server was rewritten to the same shape. `/privacy/` lists exactly those fields, and a test holds the edge config, the report's log format and the page together.
