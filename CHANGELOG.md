...
- A busy PC no longer stops a deploy in preflight. Two of the deploy runner's tests were timing the machine rather than the runner. One gave its fake step 8 seconds to start before the runner killed it, and the other failed any run slower than 20 seconds. On 2026-09-23 other work slowed the suite to about a sixth of its usual speed and the deploy stopped on both. The first now sizes its timeout from how long npm takes to start a script at that moment, and the second checks that the process holding the output outlived the run instead of timing it. The runner tests' hang limits go from 45 seconds to four minutes, since they're only there to end a hang.
