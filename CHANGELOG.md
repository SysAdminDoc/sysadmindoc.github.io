...
- The edge's own log is capped at 30 MB, as `/privacy/` says. Docker copies the host's log defaults into a container only when it creates it, and the edge was created before there were any, so its log had no size limit. The deploy's check merged the host's current defaults and passed it anyway. The edge's compose file now sets 3 files of 10 MB itself, and the check reads only what each container was created with, in Docker's decimal units (`10mb` and `10000000` are both 10 MB to Docker).
