...
- Backdrop blurs work in Chrome and Firefox again, and the scroll-progress bar and the scroll-in reveals run. The CSS minifier had no browser targets. It kept only the `-webkit-` copy of each `backdrop-filter`, which only Safari reads, and folded `animation-timeline` into the `animation` shorthand, which Chromium rejects, so the progress bar sat across the top of the homepage as a fixed line. The build now minifies for Vite's baseline browsers, adding prefixes and changing nothing else.
