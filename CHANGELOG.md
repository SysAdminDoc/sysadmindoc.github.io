...
### Added
- The CSP report stream can be read, and the nightly reads it. 262 of the first 327 reports named nothing but a directive. The sink turned every keyword a browser sends in place of a URL, such as `inline` or `eval`, into `(invalid-url)`, and it kept no sample of what was blocked. The policy now asks for `'report-sample'`, so a report about a blocked inline script or style carries its first 40 characters, and the sink scrubs anything in them shaped like an ID, a long number or an email address. It keeps those keywords and the scheme of an extension's URL (never the extension's ID), and tags each report as synthetic, extension, first-party or other. `npm run csp:reports` prints the counts. The nightly runs it after each deploy, and a first-party violation it hasn't reported before fails that run as drift once it has come in three times across two clock hours, so one burst of forged reports can't trip it. Every deploy also reads the smoke's own report back and stops unless the running sink filed it as synthetic with its sample scrubbed. `/privacy/` says what a report can hold now.
