...
- Extended the weekly quality-gates workflow to run the non-deploying `build:ci` audit path, publish endpoint/feed/search/schema statuses in the job summary, upload dedicated build-output logs, include build-output failures in quality-gate issues, and keep semantic-audit output advisory.
