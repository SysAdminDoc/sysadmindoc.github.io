...
- The endpoint audit and the nightly warn 60 days before `security.txt` expires. The audit prints the warning and still passes, the nightly writes it into its status file, and the daily health check raises it from there. Once the date has passed the audit fails the build, as it always did.
