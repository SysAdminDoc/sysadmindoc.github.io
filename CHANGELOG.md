...
- The deploy runner's own tests no longer fail just because the PC is busy. Two of them were timing the machine rather than the runner. One gave its fake step 8 seconds to start before the runner killed it, and the other failed any run slower than 20 seconds. On 2026-09-23 other work slowed the suite to about a sixth of its usual speed and a deploy stopped in preflight on both. The first now sizes its timeout from how long npm takes to start a script at that moment. The second checks that the process holding the output was still alive when the run ended, by the process's own account rather than its pid, and that the runner started its next step within 10 seconds of the step's last command (timing its "OK" line instead let a runner that logged on time and then waited 30 seconds pass), so a load that slows every start can't stretch either. The runner tests' hang limits go from 45 seconds to four minutes, since they're only there to end a hang. Preflight's browser audit still has fixed timeouts, and nobody has tried it under load yet.
