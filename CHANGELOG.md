...
- Text on `/colophon/`, `/now/` and the language pages no longer runs to the edge of a phone screen. Their content blocks had no side padding below the intro. A new browser check fails any page whose text starts within 12 pixels of either edge at phone width.
- Two layout checks had gone stale with the v0.45 redesign and failed on every run. One expected exactly two live-app cards on the homepage, which now shows up to six. The other measured a paragraph the full catalog section no longer has. They now check the six-card limit and the catalog page's own intro.
