...
- A failed request no longer copies the visitor's address and request headers into the edge server's own log. Caddy writes every 5xx there, and Docker keeps that log by size rather than age. The portfolio's error entries are dropped (its access log still records each 5xx), and every deploy reads the setting back from the running edge.
