...
- [ ] **T137** 🤖 P2 — Replace PR CI empty generated-cache stubs with schema-valid fixtures.
  - Why: T104 remains open because PR CI still satisfies generated-data imports with empty `{}` / `[]` files, so pull requests can pass without exercising README rendering, release pages, language metadata, ranking inputs, feed dates, project detail metadata, or realistic public endpoint shapes.
  ...
  - Verify: Run the fixture install/audit locally from a clean generated-cache state, then `npm test`, `npm run check`, `npm run build:ci`, `npm run endpoints:audit`, `npm run feed:audit`, `npm run search:audit`, and `npm run schema:audit`; trigger `ci.yml` with `workflow_dispatch` and confirm no empty-stub shell writes remain in logs.
